                ...
                with ui.row().classes('items-center flex-grow flex-nowrap gap-2 w-full'):
                    if options:
                        ...
                        if is_hex:
                            ...
                            inp = ui.input(type='number', label=label_text, value=v, validation={'Invalid': validate_num}).props('debounce="500"').classes('flex-grow').on_value_change(make_num_change())
                    else:
                        inp = ui.input(label=label_text, value=str(v)).classes('flex-grow').on_value_change(make_on_change())
